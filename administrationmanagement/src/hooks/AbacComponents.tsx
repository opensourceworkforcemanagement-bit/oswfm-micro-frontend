...
export function ResourceActions({
  ...
  React.useEffect(() => {
    ...
    async function checkAllPermissions() {
      ...
      const evaluationPromises = actions.map(async (action) => {
        try {
          ...
          return { id: action.id, permitted: result.decision === 'PERMIT' };
        } catch (error) {
          console.error(`Failed to evaluate permission for action ${action.id}`, error);
          return { id: action.id, permitted: false };
        }
      });
      ...
      for (const { id, permitted } of evaluationResults) {
        results[id] = permitted;
      }
